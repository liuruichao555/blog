...
<body>
    ...
    <div class="main">
        ...
        <div style="padding-top: 20px;">
            <p>标题：<input type="text" id="articleTitle" value="${requestScope.article.title}"></p>
            <p>
                分类：
                <select id="articleCategory">
                    ...
                    <c:forEach items="${requestScope.allCategory}" var="category">
                        <option
                            <c:if test="${requestScope.article.catId == category.catId}"> selected="selected" </c:if>
                            value="${category.catId}">${category.catName}</option>
                    </c:forEach>
                </select>
            </p>
            内容：<textarea id="articleContent" style="width: 100%;height: 600px;">${requestScope.article.content}</textarea><br/>
    ...
    <script type="text/javascript">
        ...
        $(function(){
            var isPost = false;
            var articleId = '${requestScope.article.articleId}';
            $('#addBtn').click(function(){
                ...
                var url = '/lrc/manager/admin/article/update';
                $.ajax({
                    url : url,
                    type : 'POST',
                    data : {'id' : articleId, 'title' : title, 'content' : content, 'catId' : catId},
                    dataType : 'json',
